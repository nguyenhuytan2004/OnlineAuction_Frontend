...
class WebSocketService {
  ...
  subscribeToAnswers(productId, questionId, handleNewAnswer) {
    ...
    this.subscriptions.set(`answers-${productId}-${questionId}`, subscription);
    return subscription;
  }
  ...
  subscribeToBlockedNotification(userId, handleBlockedNotification) {
    ...
    const subscription = this.client.subscribe(destination, (message) => {
      try {
        ...
        console.error("Error parsing private notification message:", error);
      }
    });
  ...
  subscribeToChat(conversationId, handleNewMessage) {
    if (!this.isConnected() || !this.isActive()) {
      console.error("WebSocket not connected or STOMP client not active");
      return null;
    }
    ...
    this.subscriptions.set(`chat-${conversationId}`, subscription);
    return subscription;
  }
  ...
  unsubscribeFromChat(conversationId) {
    const chatSub = this.subscriptions.get(`chat-${conversationId}`);
    if (chatSub) {
      chatSub.unsubscribe();
      this.subscriptions.delete(`chat-${conversationId}`);
    }
