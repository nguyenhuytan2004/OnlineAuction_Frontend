...
const ShippingInfoStep = ({ onNext, order, onCancel }) => {
  const {
    register,
        ...
        <form onSubmit={checkOnSubmit(handleSubmit)} className="space-y-6">
          ...
          <div className="p-5 bg-gradient-to-r from-blue-900/30 via-blue-800/20 to-blue-900/30 border border-blue-500/50 rounded-xl">
            <p className="text-blue-200 text-sm font-['Montserrat'] flex items-start gap-3">
              <span>
                Khách hàng sẽ nhận được hoá đơn vận chuyển và có thể theo dõi
                đơn hàng thông qua mã vận đơn. Đảm bảo thông tin chính xác để
                tránh lỗi giao hàng.
              </span>
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-3xl w-full max-w-md overflow-hidden border border-slate-700 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="bg-gradient-to-r from-red-600 to-red-700 p-8 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <h2 className="text-xl font-black">Xác nhận hủy đơn hàng</h2>
              </div>
              <button
            ...
            <div className="p-8 relative">
              ...
              {/* Cancellation Reason */}
              <div className="mb-6">
                <label className="block text-slate-300 font-semibold mb-2 font-['Montserrat'] text-sm">
                  Lý do hủy (tuỳ chọn)
                </label>
                <textarea
                  value={cancelReason}
                  onChange={(e) => setCancelReason(e.target.value)}
                  placeholder="Vd: Khách hàng yêu cầu hủy, không đủ hàng, v.v..."
                  rows="3"
                  className="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-600 rounded-lg text-slate-100 placeholder-slate-500 focus:border-red-500 focus:ring-2 focus:ring-red-500/30 outline-none transition-all font-['Montserrat'] resize-none"
                />
              </div>

              {/* Cảnh báo */}
              <div className="mb-6">
                <p className="text-red-500 font-['Montserrat'] text-sm bg-red-500/10 p-4 rounded-lg gap-3 border-2 border-red-500/50">
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-3 px-4 bg-slate-700 hover:bg-slate-600 text-slate-200 font-bold rounded-xl transition-colors font-['Montserrat']"
                >
                  Đóng
                </button>
                <button
                  ...
                  disabled={isLoading}
                  className="flex-1 py-3 px-4 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-bold rounded-xl transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed font-['Montserrat']"
                >
                  {isLoading ? (
                    <span className="flex items-center justify-center gap-2">
                      <svg className="animate-spin h-5 w-5" viewBox="0 0 24 24">
